...
from view.audio_renderer import AudioRenderer

class Board:
    def __init__(self):
        # 3x4 board with initial positions
        self.grid: List[List[Optional[Piece]]] = [[None for _ in range(7)] for _ in range(8)]
        self.piece_initializer = PieceInitializer()
        self.initialize_pieces()
        self.captured_pieces_player1: List[Tuple[Piece, Tuple[int, int]]] = []
        self.captured_pieces_player2: List[Tuple[Piece, Tuple[int, int]]] = []
        self.captured_positions_player1: List[Tuple[int, int]]= []
        self.captured_positions_player2: List[Tuple[int, int]]= []
        self.moved_piece = False
        self.renderer = Renderer()
        self.audio_renderer = AudioRenderer()
        self.audio_renderer.load_sound("piece move", "src/view/assets/piece1.mp3")
    ...
    def move_piece(self, start: Tuple[int, int], end: Tuple[int, int], game: "Game"):
        print("entered real move_piece")
        print(f"start[0]: {start[0]}, start[1]: {start[1]}")
        print(f"end[0]: {end[0]}, end[1]: {end[1]}")
        piece: Optional[Piece] = self.grid[start[0]][start[1]]
        if piece is not None:
            target_piece: Optional[Piece] = self.grid[end[0]][end[1]]
            ...
            if self.audio_renderer:
                self.audio_renderer.play_sound("piece move")    
    
    def get_captured_pieces(self, player: str) -> List[Tuple[Piece, Tuple[int, int]]] | None:
        if player == "Player 1":
    ...
    def get_captured_pieces_position(self, player: str):
        if player == "Player 1":
    ...
    def drop_captured_piece(self, player: str, piece: Piece, position: Tuple[int, int], game: "Game"):
    ...
    def get_invalid_dropping_points(self) -> Set[Tuple[int, int]]:
        """Returns the set of invalid dropping points."""
        self.invalid_dropping_points: Set[Tuple[int, int]] = set()  # Initialize as a set
    ...
    def get_valid_dropping_points(self, empty_cells: List[Tuple[int, int]], invalid_moves: Set[Tuple[int, int]]) -> List[Tuple[int, int]]:
        self.valid_dropping_points: Set[Tuple[int, int]] = set()
        ...
        sorted_valid_points: List[Tuple[int, int]] = sorted(self.valid_dropping_points)
        #print(sorted_valid_points)
        return sorted_valid_points

    def get_all_empty_cells(self) -> List[Tuple[int, int]]:
        empty_cells_pos: List[Tuple[int, int]] = []
        for row in range(len(self.grid)):
